    ...
    def to_representation(self, instance):
        ...
        try:
            representation['created_by'] = {
                "username": instance.created_by.username, "email": instance.created_by.email}
        except:
...
class ProductDetailSerializer(serializers.ModelSerializer):
    ...
    def get_average_rating(self, obj):
        ...
        reviews = ProductReview.objects.filter(
            product=obj, approval_status=ProductReviewApprovalStatusChoices.APPROVED)
        total_rating = 0
        if reviews.count() > 0:
    ...
    def get_total_reviews(self, obj):
        total_count = ProductReview.objects.filter(
            product=obj, approval_status=ProductReviewApprovalStatusChoices.APPROVED).count()
        if total_count == 0:
            return 0
        review_list = {
...
class ProductReviewSerializer(serializers.ModelSerializer):
    review_by = serializers.HiddenField(
        default=serializers.CurrentUserDefault())
    created_at = serializers.DateTimeField(
        format='%d/%m/%y %H:%M', required=False)
    ...
    def validate_rating(self, value):
        if value < 1 or value > 5:
            raise serializers.ValidationError(
                "Rating must be between one star and five star!")
        return value
